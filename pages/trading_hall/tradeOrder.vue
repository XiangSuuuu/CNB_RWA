	...
	<view class="pages">
		...
		<view class="shop-wrap">
			<view class="shop-item" v-for="(item, index) in lists" :key="index">
				...
				<view style="width: 100%; height: 2rpx; background-color: #655A8980;"></view>
				<view class="flex-lr lists">
					<view style="color: #FFFFFF; font-weight: 400; font-family: 'Open Sans'; font-size: 24rpx;">{{formattedDate(item.timestamp*1000)}}</view>
					<view class="flex-r">
						<!-- <view>限额</view>
						<view class="flex-r">
							<view class="title-num">0.009</view>
							{{$Unit}}
						</view> -->
						<view class="min-btn" @click="cancelSellOrder(item)">{{$t('index._TEXT151')}}</view>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			init() {
				try{
					this.$c2ctcc.getUserSellOrder(this.myAddress).then((res) => {
						if (res) {
							this.lists = res;
							console.log(res)
						} 
